...
class VideoDanmuProvider: DanmuProviderProtocol {
    ...
    func fetchDanmuView() async {
        ...
        do {
            reply = try await ApiRequest.requestPB(QApi.danmuWebView, parameters: ["type": 1, "oid": cid!])
        } catch let err {
            print("[dm] cid:\(cid!) requestDanmuWebView error: \(err)")
            return
        }
        ...
        print("[dm] cid:\(cid!) up danmu cnt: \(dms.count)")
    }

    func fetchDanmuList(_ idx: Int) async {
        var reply: DmSegMobileReply
        do {
            reply = try await ApiRequest.requestPB(QApi.danmuList, parameters: ["type": 1, "oid": cid!, "segment_index": idx])
        } catch let err {
            segmentStatuses[idx] = nil // 等待下次重试
            print("[dm] cid:\(cid!) sidx:\(idx) requestDanmuList error: \(err)")
            return
        }
        ...
        print("[dm] cid:\(cid!) sidx:\(idx) danmu cnt: \(dms.count)")
    }
    ...
    private func fetchMoreDanmuInBackground(time: TimeInterval) {
        func fetchDanmuInBackground(_ idx: Int) {
            ...
            print("[dm] cid:\(cid!) time:\(Int(time)) fetching sidx:\(idx)")
        }
