...
class QFreshHeader: MJRefreshHeader {
    ...
    lazy var loadingActivity: UIActivityIndicatorView = {
        let loadingActivity = UIActivityIndicatorView(style: UIActivityIndicatorView.Style.medium)
    ...
    fileprivate func rotaArrow() {
        UIView.animate(withDuration: 0.2, animations: {
            self.arrowImageView.transform = self.arrowImageView.transform.rotated(by: Double.pi)
        }) { (complete) in
        }
    }
