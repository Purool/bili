...
//  bili
//
//  Created by Purool on 15/8/2024.
//
...
enum QApi {
    ...
    static let recommendListWeb = "/x/web-interface/index/top/feed/rcmd"
    ...
    static let hasLikeVideo = "/x/web-interface/archive/has/like"
    ...
    static let hasCoinVideo = "/x/web-interface/archive/coins"
    ...
    static let favVideo = "/x/v3/fav/resource/deal"
    
    // 判断视频是否被收藏（双端）GET
    /// aid
    // https://api.bilibili.com/x/v2/fav/video/favoured
    static let hasFavVideo = "/x/v2/fav/video/favoured"
    ...
    // bvid    str    稿件bvid    必要（可选）    avid与bvid任选一个
    // csrf    str    CSRF Token（位于cookie）    必要
    static let oneThree = "/x/web-interface/archive/like/triple"
    ...
    // rid    num    目标 视频稿件avid
    static let videoInFolder = "/x/v3/fav/folder/created/list-all"
    
    // 视频详情页 相关视频
    static let relatedList = "/x/web-interface/archive/related"
    
    // 查询用户与自己关系_仅查关注
    static let hasFollow = "/x/relation"
    
    // 操作用户关系
    static let relationMod = "/x/relation/modify"
    
    // 相互关系查询 // 失效
    // static let relationSearch = "/x/space/wbi/acc/relation"
    ...
    // 评论点赞
    static let likeReply = "/x/v2/reply/action"
    ...
    static let userStat = "/x/relation/stat"
    
    // 获取用户信息
    static let userInfo = "/x/web-interface/nav"
    
    // 获取当前用户状态
    static let userStatOwner = "/x/web-interface/nav/stat"
    
    // 收藏夹
    // https://api.bilibili.com/x/v3/fav/folder/created/list?pn=1&ps=10&up_mid=17340771
    static let userFavFolder = "/x/v3/fav/folder/created/list"
    ...
    /// ps int pageSize
    /// keyword let 搜索词
    /// order let 排序方式 view 最多播放 mtime 最近收藏 pubtime 最近投稿
    /// tid int 分区id
    /// platform web
    ...
    static let userFavFolderDetail = "/x/v3/fav/resource/list"
    
    // 正在直播的up & 关注的up
    // https://api.bilibili.com/x/polymer/web-dynamic/v1/portal
    static let followUp = "/x/polymer/web-dynamic/v1/portal"
    ...
    static let followDynamic = "/x/polymer/web-dynamic/v1/feed/all"
    
    // 动态点赞
    static let likeDynamic =
    "\(HttpString.tUrl)/dynamic_like/v1/dynamic_like/thumb"
    
    // 获取稍后再看
    static let seeYouLater = "/x/v2/history/toview"
    
    // 获取历史记录
    static let historyList = "/x/web-interface/history/cursor"
    
    // 暂停历史记录
    static let pauseHistory = "/x/v2/history/shadow/set"
    
    // 查询历史记录暂停状态
    static let historyStatus = "/x/v2/history/shadow?jsonp=jsonp"
    
    // 清空历史记录
    static let clearHistory = "/x/v2/history/clear"
    
    // 删除某条历史记录
    static let delHistory = "/x/v2/history/delete"
    
    // 搜索历史记录
    static let searchHistory = "/x/web-goblin/history/search"
    
    // 热搜
    static let hotSearchList =
    ...
    // 搜索关键词
    static let searchSuggest =
    "https://s.search.bilibili.com/main/suggest"
    
    // 分类搜索
    static let searchByType = "/x/web-interface/search/type"
    ...
    static let ab2c = "/x/player/pagelist"
    ...
    static let tagFollowings = "/x/relation/tag"
    
    // 关注分类
    // https://api.bilibili.com/x/relation/tags
    static let followingsClass = "/x/relation/tags"
    
    // 搜索follow
    static let followSearch = "/x/relation/followings/search"
    ...
    // wts=1689767832
    static let memberArchive = "/x/space/wbi/arc/search"
    
    // 用户动态搜索
    static let memberDynamicSearch = "/x/space/dynamic/search"
    
    // 用户动态
    static let memberDynamic = "/x/polymer/web-dynamic/v1/feed/space"
    
    // 稍后再看
    static let toViewLater = "/x/v2/history/toview/add"
    
    // 移除已观看
    static let toViewDel = "/x/v2/history/toview/del"
    
    // 清空稍后再看
    static let toViewClear = "/x/v2/history/toview/clear"
    
    // 追番
    static let bangumiAdd = "/pgc/web/follow/add"
    
    // 取消追番
    static let bangumiDel = "/pgc/web/follow/del"
    ...
    // 我的订阅
    static let bangumiFollow =
    "/x/space/bangumi/follow/list?type=1&follow_status=0&pn=1&ps=15&ts=1691544359969"
    
    // 黑名单
    static let blackLst = "/x/relation/blacks"
    
    // 移除黑名单
    static let removeBlack = "/x/relation/modify"
    
    // github 获取最新版
    static let latestApp =
    ...
    // up主分组
    static let followUpTag = "/x/relation/tags"
    
    // 设置Up主分组
    // 0 添加至默认分组  否则使用,分割tagid
    static let addUsers = "/x/relation/tags/addUsers"
    
    // 获取指定分组下的up
    static let followUpGroup = "/x/relation/tag"
    ...
    /// w_rid
    /// wts
    static let getRecentCoinVideoApi = "/x/space/coin/video"
    
    /// 最近点赞的视频
    static let getRecentLikeVideoApi = "/x/space/like/video"
    
    /// 最近追番
    static let getRecentBangumiApi = "/x/space/bangumi/follow/list"
    
    /// 用户专栏
    static let getMemberSeasonsApi = "/x/polymer/web-space/home/seasons_series"
    
    /// 获赞数 播放数
    /// mid
    static let getMemberViewApi = "/x/space/upstat"
    ...
    /// page_num
    /// page_size
    static let getSeasonDetailApi =
    "/x/polymer/web-space/seasons_archives_list"
    
    /// 获取未读动态数
    static let getUnreadDynamic = "/x/web-interface/dynamic/entrance"
    
    /// 用户动态主页
    static let dynamicSpmPrefix = "https://space.bilibili.com/1/dynamic"
    
    /// 激活buvid3
    static let activateBuvidApi = "/x/internal/gaia-gateway/ExClimbWuzhi"
    
    /// 获取字幕配置
    static let getSubtitleConfig = "/x/player/v2"
    
    /// 我的订阅
    static let userSubFolder = "/x/v3/fav/folder/collected/list"
    
    /// 我的订阅详情
    static let userSubFolderDetail = "/x/space/fav/season/list"
    
    /// 表情
    static let emojiList = "/x/emote/user/panel/web"
    
    /// 已读标记
    static let ackSessionMsg =
    "\(HttpString.tUrl)/session_svr/v1/session_svr/update_ack"
    
    /// 发送私信
    static let sendMsg = "\(HttpString.tUrl)/web_im/v1/web_im/send_msg"
    
    /// 排行榜
    static let getRankApi = "/x/web-interface/ranking/v2"
    
    /// 取消订阅
    static let cancelSub = "/x/v3/fav/season/unfav"
}
